...
# Edited: Samuel Krizan, xkriza06@stud.fit.vutbr.cz
...
LOG=false
LANG=sk
DUMP_PATH=/mnt/minerva1/nlp/corpora/monolingual/slovak/wikipedia/
DUMP_VERSION=latest
...
usage() {
    ...
    if test $cut_DUMP_PATH -lt ${#DUMP_PATH}; then
...
while [ "$1" != "" ]; do
    ...
    --log)
        ...
        ;;
    *)
        echo >&2 "ERROR: unknown parameter \"$PARAM\""
        usage
        exit 1
        ;;
    esac
...
if test -L "${DUMP_PATH}"; then
    DUMP_PATH=$(readlink "${DUMP_PATH}")
    if test $(dirname "${DUMP_PATH}") = "."; then
...
"sk")
    cd ${LANG}
    CMD="python3 wiki_sk_extract.py --lang ${LANG} --dump ${DUMP_VERSION} --indir \"${DUMP_PATH}\" ${MULTIPROC_PARAMS} 2>entities_processing.log"
    echo "RUNNING COMMAND: ${CMD}"
    eval $CMD
    if $LOG; then
        metrics_params="--log"
...
python3 kbwiki2gkb.py --outdir outputs --indir outputs
