  ...
  with open(in_head, 'r', encoding = 'utf8') as fin_head:
    for line in fin_head:
      ...
      if ent_type:
        ...
        line = re.sub(r'{.*?}', '', line)
        line = re.sub(r'\[.*?\]', '', line)
        in_columns[ent_type.lower()] = {key : i for i, key in enumerate(line.split('\t'))}
        if not col_type:
...
parser = argparse.ArgumentParser(description = 'Convert KB in wikipedia format to generic KB format.')
parser.add_argument('--indir', default = '.', help = 'Input files (wikipedia format) directory path (default: %(default)s).')
