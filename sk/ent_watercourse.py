...
class EntWatercourse(EntCore):
    ...
    def __init__(self, title, prefix, link, redirects, langmap):
        ...
        """
        super(EntWatercourse, self).__init__(
    ...
    def is_watercourse(title, content):
        ...
        """
        # kontrola šablon
        if re.search(r"{{\s*Infobox\s*Potok", content, re.I):
            return 1, "Infobox 'vodní tok'"

        if re.search(r"{{\s*Geobox\s*\|\s*(River|Stream|Creek)", content, re.I):
            return 1, "Infobox 'vodní tok'"
        ...
        # kontrola názvu
        rexp = re.search(
            r"\((?:bystrina|potok|riečka|rieka|veľtok|prietok)\)$", title)
        if rexp:
            return 3, "Nazov '" + rexp.group(0) + "'"
    ...
    def line_process_infobox(self, ln, is_infobox_block):
        ...
        # délka toku
        rexp = re.search(
            r"(?:Dĺžka\s*toku|length)\s*=(?!=)\s*((?:.+?)(?=\|[^\|]*\=)|(?:.*))", ln, re.I)
        if rexp and rexp.group(1):
        ...
        # plocha
        rexp = re.search(
            r"(?:Plocha\s*povodia|watershed)\s*=(?!=)\s*((?:.+?)(?=\|[^\|]*\=)|(?:.*))", ln, re.I)
        if rexp and rexp.group(1):
        ...
        # světadíl
        rexp = re.search(
            r"(?:Svetadiel|continent)\s*=(?!=)\s*((?:.+?)(?=\|[^\|]*\=)|(?:.*))", ln, re.I)
        if rexp and rexp.group(1):
        ...
        # průtok
        rexp = re.search(
            r"(?:Prietok|discharge)\s*=(?!=)\s*((?:.+?)(?=\|[^\|]*\=)|(?:.*))", ln, re.I)
        if rexp and rexp.group(1):
    ...
    def line_process_1st_sentence(self, ln):
        ...
        rexp = re.search(
            r".*?'''.+?'''.*?\s(?:bol[aiy]?|je|sú|nachádz(?:a|ajú)|patr(?:í|il|ila|ilo)|stal|rozprestier(?:a|al)|lež(?:í|al)|pramen(?:í|il)).*?"
            + abbrs
            + "\.(?![^[]*?\]\])",
            ln,
        )
