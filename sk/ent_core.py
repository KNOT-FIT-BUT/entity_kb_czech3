...
class EntCore(metaclass=ABCMeta):
    ...
    def __init__(self, title, prefix, link, redirects, langmap):
        ...
        self.aliases_infobox_sk = DictOfUniqueDict()
        self.aliases_infobox_orig = DictOfUniqueDict()
        self.description = ""
        self.images = ""
        self.n_marked_slovak = 0
    ...
    def del_redundant_text(text, multiple_separator="|", langmap=dict()):
        ...
        if link_lang and link_lang.group(1):
            ...
            if txt_lang in langmap:
                text = text.replace(
                    link_lang.group(0), "{{{{v jazyk|{}}}}} ".format(
                        langmap[txt_lang])
                )
        clean_text = re.sub(
            r"\[\[[^\]|]+\|([^\]|]+)\]\]", r"\1", text
        ...
        clean_text = re.sub(
            r"\s*{{(?:(?:doplňte|doplniť|chýba) zdroj|zdroj\?|fakt[^}]*)}}\s*",
            "",
            clean_text,
            flags=re.I,
        )
    ...
    def get_data(self, content):
        ...
        * content - content of the page (str)
        ...
        try:
            ...
            is_infobox = False
            # True = Block infobox / False = Inline (or combined) infobox / None = Unknown
            ...
            for ln in data:
                ...
                rexp = re.search(self.re_infobox_kw_img +
                                 r"\s*=(?!=)\s*(.*)", ln, re.I)
                ...
                if not is_infobox:
                    ...
                    infobox_data = re.search(
                        r"{{(Infobox|Geobox)([^\|]+)(\|)?.*$", ln, flags=re.I
                    )
                    if infobox_data:
                        self.infobox_type = (
                        ...
                        if re.search(
                            r"Svetové\s+dedičstvo", infobox_data.group(1), flags=re.I
                        ):
                            is_infobox_unesco = True
                        ln = infobox_data.group(0)
                        # Empty infobox
                        ln_infobox_start = re.search(
                            r"(Infobox|Geobox)[^{]*}}\s*([^\s].*(?!}}))?$", ln, flags=re.I
                        )
                        if ln_infobox_start:
                ...
                if is_infobox:
                    ...
                    if not was_infobox:
                        ...
                        if not is_infobox_unesco:
                            self.line_process_infobox(
                                part_infobox, is_infobox_block)
                            if infobox_braces_depth == 0:
                                was_infobox = True
    ...
    def get_aliases(self, alias, marked_slovak=False, nametype=None):
        """
        ...
        re_lang_aliases = re.compile(
            "{{(?:Cjz|Vjz|Lang|v\s*jazyku|cudzojazyčne?)\|(?:\d=)?(\w+)\|(?:\d=)?([^}]+)}}",
            flags=re.I,
        )
        re_lang_aliases2 = re.compile(
            "{{v\s*jazyku\|(\w+)}}\s+([^{]{2}.+)", flags=re.I)
        ...
        alias = re.sub(
            r"\s*{{Poznámka pod čiarou.*(?:}})?\s*$", "", alias, flags=re.I)
        alias = re.sub(
            r"\s*\{{Unicode\|([^}]+)}}\s*", r" \1", alias, flags=re.I)
        alias = re.sub(
            r"\s*\({{(?:Cjz|Vjz|Lang|v\s*jazyku|cudzojazyčne?)\|(?:\d=)?\w+\|(?:\d=)?[^}]+}}\)\s*",
            "",
            alias,
            flags=re.I,
        ...
        alias = re.sub(
            r"\s*\({{(?:Cjz|Vjz|Lang|v\s*jazyku|cudzojazyčne?)\|\w+\|[^}]+}}\)\s*", "", alias, flags=re.I
        )  # aliases are covered by "lang_aliases"
        alias = re.sub(
            r"\s*\(?{{(?:Cjz|Vjz|Lang|v\s*jazyku|cudzojazyčne?)\|\w+}}\)?:?\s*", "", alias, flags=re.I
        )  # aliases are covered by "lang_aliases"
        alias = re.sub(
            r"\s*\(?{{(?:Cjz|Vjz|Lang|v\s*jazyku|cudzojazyčne?)\|[\w-]+\|([^}]+)}}\)?:?\s*",
            r"\1",
            alias,
            flags=re.I,
        )
        alias = re.sub(r"\s*{{(?:Malé|Veľké)\|(.*?)}}\s*",
                       r"\1", alias, flags=re.I)
        if re.search(r"\s*{{Možno hľadáte", alias, flags=re.I):
            alias = re.sub(
                r"\s*{{Možno hľadáte|([^=|])*?}}\s*", r"\1", alias, flags=re.I
            )
            alias = re.sub(
                r"\s*{{Možno hľadáte|.*?iné\s*=\s*([^|])*?.*?}}\s*",
                r"\1",
                alias,
                flags=re.I,
            )
        ...
        for a in alias.split("|"):
            ...
            for av in self.unfold_alias_variants(a):
                if re.search(r"[^\W_/]", av):
                    if marked_slovak:
                        result.update(
                            self.scrape_quoted_inside(
                                av, nametype, self.LANG_SLOVAK)
                        )
                        self.n_marked_slovak += 1
    ...
    def serialize_aliases(self):
        ...
        """
        self.aliases.update(self.aliases_infobox_sk)
        ...
        if (
            ...
            self.aliases[self.first_alias][KEY_LANG] = self.LANG_SLOVAK
    ...
    def get_image(self, image):
        ...
        image = (
            re.sub(r"\s*\|.*$", "", image).replace("}",
                                                   "").strip().replace(" ", "_")
        )
        image_hash = md5(image.encode("utf-8")).hexdigest()[:2]
        image = "wikimedia/commons/" + \
            image_hash[0] + "/" + image_hash + "/" + image
